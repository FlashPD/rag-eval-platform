...
def test_settings_validate_api_key_hashes() -> None:
    digest = "a" * 64

    assert Settings(_env_file=None, api_key_hashes=(digest,)).api_key_hashes == (digest,)

    with pytest.raises(ValidationError, match="lowercase SHA-256"):
        Settings(_env_file=None, api_key_hashes=("not-a-digest",))

    with pytest.raises(ValidationError, match="must be unique"):
        Settings(_env_file=None, api_key_hashes=(digest, digest))
